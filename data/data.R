...
data <- read.csv2("Base/SearchResults.csv", sep = ",", stringsAsFactors = T)
...
dt1<-function(x){
  a1<- chartr("áàãâéèíóöúûüñçýõòìîžêëäšℓτχy","aaaaeeioouuuncyooiizeeasltxy",x)
  return(a1)
}
...
# CORRECIÓN VARIABLE TÍTULOS==================================

titulos <- data.frame(data_clean4$NCT.Number,data_clean4$Title,data_clean4$URL)

# write.table(titulos, "titulos.csv" , sep = "|", row.names = F,col.names = T)
...
data_clean5 <- data_clean4 %>% 
              select(Title,
                     ...
                     pais,
                     URL) %>% 
               plyr::rename(c("Study.Results"="Study_Results",
...
indice <- sample( 1:nrow(data_clean5 ),25)

indice_muestreado <- data_clean5[indice, ]

save(indice_muestreado, file="indice_muestreado.RData")

# ==============================================================================================================

# PARA RELACIONAR PAISES

relaciones <- data_clean2 %>% 
              filter(Locations!="Desconocido") %>% 
              select(NCT.Number,
                     Title,
                     Status,
